...
/**
 * 离线级联图界面的输入资源选择对话框， 用于获取站点,实验名等信息
 * 
 * @author LYNN
 * @version 1.0,14/12/24
 * @since JDK1.625
 */
public abstract class CascadPropDialog extends PropDialog
{

	public CascadPropDialog(Vector<WorkSpaceProp> workSpaceProps)
	{
		// TODO Auto-generated constructor stub
		super(workSpaceProps, 500, 147);
	}
	...
	void addItem(int id)
	{
		...
		for (int i = 0; i < workSpaceProps.get(id).allAinCfgInfos.get(0).channelName.length; i++)
		{
			String prop = workSpaceProps.get(id).allAinCfgInfos.get(0).channelName[i];
			refBox.addItem(prop);
		}
	...
	protected void IntemInit()
	{
		...
		item = new String[] { 
				...
				HduChartUtil.getResource("OfflinePropDialog_RevChannel")
				 };
	}
