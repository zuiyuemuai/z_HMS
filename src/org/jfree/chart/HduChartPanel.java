...
import org.jfree.chart.panel.HduOverlay;
...
import com.nari.slsd.hms.hdu.offline.multiICell.waveAnalyse.FloatMouseListener;
 ...
 */
public class HduChartPanel extends JPanel implements ChartChangeListener,
		...
		MouseMotionListener, OverlayChangeListener, Printable, Serializable
{
	...
	/*********************** start *********************************/
	/** 原波形图平移命令 */
	private final String MOVEO_COMMAND = "MOVEO";
	/** 原波形图撤销命令 */
	private final String BACKO_COMMAND = "BACKO";
	/** 重绘图形命令 */
	private final String RECOVER_COMMAND = "RECOVER";

	private UndoManager undoManager = new UndoManager();

	public static final int LINE_MOVE = 1001;// 移动
	public static final int LINE_SCALE_X = 1002;// X放大
	public static final int LINE_SCALE_Y = 1003;// Y放大
	public static final int LINE_SCALE = 1004;// both
	public static final int SHOW_JIANXIANG = 1007;// 键像点整周期fft
	/*
	 * 
	 */
	public int mouseDragOperation = LINE_SCALE;

	public static double pressPointX; // 鼠标按下后获取的X坐标
	public static double pressPointY; // 鼠标按下后获取的Y坐标
	public static double releasePointX; // 鼠标松开后获取的X坐标
	public static double releasePointY; // 鼠标松开后获取的Y坐标
	public double[] pressPoint;
	public double[] releasePoint;
	public org.jfree.data.Range nowRange;

	public int xline;
	public int yline;
	public double MovPointX;
	public double MovPointY;
	private Boolean xread = false;
	private Boolean yread = false;
	private Boolean ifMouseDragged = false;
	 ...
	 *            <code>false</code>).
	 */
	public HduChartPanel(JFreeChart chart, boolean useBuffer)
	{

		this(chart, DEFAULT_WIDTH, DEFAULT_HEIGHT, DEFAULT_MINIMUM_DRAW_WIDTH,
				DEFAULT_MINIMUM_DRAW_HEIGHT, DEFAULT_MAXIMUM_DRAW_WIDTH,
				DEFAULT_MAXIMUM_DRAW_HEIGHT, useBuffer, false, // properties
				true, // save
				false, // print
				true, // zoom
				true // tooltips
	 ...
	 *            for the chart.
	 */
	public HduChartPanel(JFreeChart chart, boolean properties, boolean save,
	 ...
	 *            for the chart.
	 */
	public HduChartPanel(JFreeChart chart, int width, int height,
			...
			boolean save, boolean print, boolean zoom, boolean tooltips)
	{
		...
		this(chart, width, height, minimumDrawWidth, minimumDrawHeight,
				...
				false, save, print, zoom, tooltips);
	}
			...
			boolean tooltips)
	{
		...
		// setPreferredSize(new Dimension(width, height)); 注释
	 ...
	 * @param fillRectangle
	 *            <code>true</code> if zoom rectangle is filled, false if
	 *            rectangle is shown as outline only.
	 */
	...
	public void setMouseWheelEnabled(boolean flag)
	{
		if (flag && this.mouseWheelHandler == null)
		{
			...
			try
			{
				...
				Constructor cc = c
						.getConstructor(new Class[] { HduChartPanel.class });
		...
		} else
		{
			...
			if (this.mouseWheelHandler != null)
			{
				...
				{
					Class mwl = Class
							.forName("java.awt.event.MouseWheelListener");
					Class c2 = HduChartPanel.class;
	 ...
	 * @since 1.0.13
	 */
	public void addOverlay(HduOverlay overlay)
	{
		if (overlay == null)
		{
	 ...
	 * @since 1.0.13
	 */
	public void removeOverlay(HduOverlay overlay)
	{
		if (overlay == null)
		{
			throw new IllegalArgumentException("Null 'overlay' argument.");
	...
	public String getToolTipText(MouseEvent e)
	{
		...
		mousePoint = BaseChartJpanel.translateScreenToValue(this,
				new Point(e.getX(), e.getY()));
		float x = (float) Math.round(mousePoint[0] * 100) / 100;
		float y = (float) Math.round(mousePoint[1] * 100) / 100;

		return "(" + x + "," + y + ")";
		/*
		 * String result = null;
		 * 
		 * if (this.info != null) { EntityCollection entities =
		 * this.info.getEntityCollection(); if (entities != null) { Insets
		 * insets = getInsets(); ChartEntity entity = entities.getEntity( (int)
		 * ((e.getX() - insets.left) / this.scaleX), (int) ((e.getY() -
		 * insets.top) / this.scaleY)); if (entity != null) { result =
		 * entity.getToolTipText(); } } } return result;
		 */
	 ...
	 * 
	 * @param flag
	 *            <code>true</code> indicates that the buffer should be
	 *            refreshed.
	 */
	...
	public void paintComponent(Graphics g)
	{
		...
		Rectangle2D available = new Rectangle2D.Double(insets.left, insets.top,
				size.getWidth() - insets.left - insets.right, size.getHeight()
		...
		if (this.useBuffer)
		{
			...
			if (this.refreshBuffer)
			{
				...
				Graphics2D bufferG2 = (Graphics2D) this.chartBuffer
						.getGraphics();
				...
				if (scale)
				{
					...
					this.chart
							.draw(bufferG2, chartArea, this.anchor, this.info);
		...
		{
			HduOverlay overlay = (HduOverlay) iterator.next();
	...
	public void actionPerformed(ActionEvent event)
	{
		...
		} else if (command.equals(MOVEO_COMMAND))
		{
			mouseDragOperation = LINE_MOVE;
		} else if (command.equals(BACKO_COMMAND))
		{
		...
		} else if (command.equals(RECOVER_COMMAND))
		{
	...
	public void mousePressed(MouseEvent e)
	{
		pressPointX = e.getX();
		pressPointY = e.getY();
		pressPoint = translateScreenToValue(new Point((int) pressPointX,
				(int) pressPointY));
		MovPointX = pressPointX;
		MovPointY = pressPointY;
			...
			if (plot instanceof Pannable)
			{
				...
				if (pannable.isDomainPannable() || pannable.isRangePannable())
				{
					...
					if (screenDataArea != null
							&& screenDataArea.contains(e.getPoint()))
					{
	...
	public double[] translateScreenToValue(Point point)
	{
		double[] valuePoint = new double[2];
	...
	public void mouseDragged(MouseEvent e)
	{
		...
		} else if (hZoom)
		{
			double xmax = Math.min(e.getX(), scaledDataArea.getMaxX());
			this.zoomRectangle = new Rectangle2D.Double(this.zoomPoint.getX(),
					scaledDataArea.getMinY(), xmax - this.zoomPoint.getX(),
		...
		ValueAxis r = plot.getRangeAxis();
		nowRange = d.getRange();
		double[] mousePoint = new double[2];
		Graphics2D graphics2D = (Graphics2D) getGraphics();
		if (ifMouseDragged)
		{
			ifMouseDragged = false;
			Line lineModel = new Line();
			lineModel.setXLowerBound(d.getLowerBound());
			lineModel.setXUpperBound(d.getUpperBound());
			lineModel.setYLowerBound(r.getLowerBound());
			lineModel.setYUpperBound(r.getUpperBound());

			UnRedoEdit unRedoEdit = new UnRedoEdit(chart.getXYPlot());
			unRedoEdit.addLine(lineModel);

			undoManager.addEdit(unRedoEdit);

		}

		mousePoint = translateScreenToValue(new Point(e.getX(), e.getY()));
		...
		case LINE_MOVE:
		{
			...
			double m = e.getX(); // 获取当前坐标
			double n = e.getY();
			double nowPoint[] = translateScreenToValue(new Point((int) m,
					(int) n));
			double lastPoint[] = translateScreenToValue(new Point(
					(int) MovPointX, (int) MovPointY));
			double xd = m - MovPointX; // 当前坐标减去上一次的坐标
			double yd = n - MovPointY;
			double xd1 = nowPoint[0] - lastPoint[0];
			double yd1 = nowPoint[1] - lastPoint[1];
			MovPointX = MovPointX + xd; // 动态变换上一次坐标
			MovPointY = MovPointY + yd;

			// 因为获取的坐标值和真实的坐标值有出入，所以要配好参数，才能更的达到平移效果
			d.setRange(d.getLowerBound() - xd1, d.getUpperBound() - xd1);// 改变横轴坐标范围
			r.setRange(r.getLowerBound() - yd1, r.getUpperBound() - yd1);// 改变纵轴坐标范围
		...
		case LINE_SCALE_X:
		{
			// firtstpointX = this.zoomPoint.getX();
			// firtstpointY = this.zoomPoint.getY();
			// this.setDomainZoomable(true);
			// this.setRangeZoomable(false);
	...
	class UnRedoEdit extends AbstractUndoableEdit
	{
		...
		public UnRedoEdit(XYPlot plot)
		{
			this.plot = plot;
			index = 0;
		}

		public void addLine(Line lineModel)
		{
			model.addElement(lineModel);
		...
		public void undo() throws CannotUndoException
		{
			...
			Line lineModel2 = (Line) model.getElementAt(index);

			ValueAxis domaAxis = plot.getDomainAxis();
			ValueAxis rangeAxis = plot.getRangeAxis();
			domaAxis.setRange(lineModel2.getXLowerBound(),
					lineModel2.getXUpperBound());
			rangeAxis.setRange(lineModel2.getYLowerBound(),
					lineModel2.getYUpperBound());
			super.undo();
		}
	...
	public void mouseReleased(MouseEvent e)
	{
		...
		releasePointX = e.getX();
		releasePointY = e.getY();
		releasePoint = translateScreenToValue(new Point((int) releasePointX,
				(int) releasePointY));
			...
			if (zoomTrigger1 || zoomTrigger2)
			{
				...
				} else
				{
					...
					if (!vZoom)
					{
						...
						w = Math.min(this.zoomRectangle.getWidth(), maxX
								- this.zoomPoint.getX());
		...
		// releasePointX = e.getX();
		// releasePointY = e.getY();
		// double[] lastPoint=translateScreenToValue(new
		// Point((int)releasePointX,(int)releasePointY));
		// if (mouseDragOperation==SHOW_JIANXIANG)
		// {
		// jianxiang(lastPoint);
		// }
		/******************** end *****************************/
	...
	public void zoom(Rectangle2D selection)
	{
		...
		Point2D selectOrigin = translateScreenToJava2D(new Point(
				(int) Math.ceil(selection.getX()), (int) Math.ceil(selection
						.getY())));
	 ...
	 * in PNG format. changed by CHL on August 31st,2014
	 * 
	 * @throws IOException
	...
	{
		// ResourceBundle localizationResources = ResourceBundleWrapper
		// .getBundle("org.jfree.chart.chart");

		JFileChooser fileChooser = new JFileChooser();
		fileChooser.setCurrentDirectory(this.defaultDirectoryForSaveAs);
		ExtensionFileFilter filter = new ExtensionFileFilter(
				HduChartUtil.getResource("PNG_Image_Files"), ".png");
		ExtensionFileFilter filter2 = new ExtensionFileFilter(
				HduChartUtil.getResource("JPEG_Image_Files"), ".jpg");

		fileChooser.addChoosableFileFilter(filter2);
		fileChooser.addChoosableFileFilter(filter);
		fileChooser.getFileFilter();

		String jpegSelected = HduChartUtil.getResource("JPEG_Image_Files");
		...
		if (option == JFileChooser.APPROVE_OPTION)
		{
			...
			if (fileFilter.getDescription().equals(jpegSelected))
			{
				...
				ChartUtilities.saveChartAsJPEG(new File(filename), this.chart,
						getWidth(), getHeight());
			} else
			{
				System.out.println(fileFilter.getDescription());
	 ...
	 * @param pageIndex
	 *            the index of the page. If not <code>0</code>, nothing gets
	 *            print.
	 * 
	 ...
	 * @return The popup menu.
	 */
	public JPopupMenu createPopupMenu(boolean properties, boolean save,
			boolean print, boolean zoom, boolean ifmove, boolean ifback,
			boolean ifrepain, boolean isadjust)
	{
		return createPopupMenu(properties, false, save, print, zoom, ifmove,
				ifback, ifrepain, isadjust);
	}
	 ...
	 * @param isseparator
	 */
	public void addPopupMenuItem(JPopupMenu result, JMenuItem item,
			boolean isseparator)
	{
		result.add(item);
		if (isseparator)
	 ...
	 *            是否在下面画分割线
	 */
	public void addMenuItem_Properties(JComponent result)
	{
		JMenuItem propertiesItem = new JMenuItem(
				HduChartUtil.getResource("Properties..."));
	...
	public void addMenuItem_Save(JComponent result)
	{
	...
	public void addMenuItem_Back(JComponent result)
	{
		// 新建一个"撤銷"的在菜单项目
		JMenuItem back = new JMenuItem(HduChartUtil.getResource("Back"));
		back.setActionCommand(BACKO_COMMAND);
		back.addActionListener(this);
		result.add(back);

	}

	public void addMenuItem_Zoom(JComponent result)
	{
		JMenu zoomInMenu = new JMenu(HduChartUtil.getResource("Zoom_In"));

		this.zoomInBothMenuItem = new JMenuItem(
				HduChartUtil.getResource("All_Axes"));
		...
		this.zoomInDomainMenuItem = new JMenuItem(
				HduChartUtil.getResource("Domain_Axis"));
		...
		this.zoomInRangeMenuItem = new JMenuItem(
				HduChartUtil.getResource("Range_Axis"));
	...
	public void addMenuItem_Adjust(JComponent result)
	{
		JMenu autoRangeMenu = new JMenu(HduChartUtil.getResource("Auto_Range"));
		...
		this.zoomResetDomainMenuItem = new JMenuItem(
				HduChartUtil.getResource("Domain_Axis"));
		this.zoomResetDomainMenuItem
				.setActionCommand(ZOOM_RESET_DOMAIN_COMMAND);
	...
	public void addMenuItem_Recover(JComponent result)
	{
		// 新建一个"重繪"的在菜单项目
		JMenuItem recover = new JMenuItem(HduChartUtil.getResource("Recover"));
		recover.setActionCommand(RECOVER_COMMAND);
		recover.addActionListener(this);
		result.add(recover);
	}

	public void addMenuItem_Move(JComponent result)
	{
		// 新建一个"平移"的在菜单项目
		final JCheckBoxMenuItem move = new JCheckBoxMenuItem(
				HduChartUtil.getResource("Move"));
		move.setActionCommand(MOVEO_COMMAND);
		result.add(move);
		move.addItemListener(new ItemListener()
		{
			...
			public void itemStateChanged(ItemEvent e)
			{
				...
				{
					mouseDragOperation = FloatMouseListener.NOTHING;
				}
			}
		});
	}

	public void addMenuItem_Copy(JComponent result)
	{
		JMenuItem copyItem = new JMenuItem(HduChartUtil.getResource("Copy"));
	...
	public void addMenuItem_Print(JComponent result)
	{
		JMenuItem printItem = new JMenuItem(
				HduChartUtil.getResource("Print..."));
	 ...
	 * @since 1.0.13
	 */
	public JPopupMenu createPopupMenu(boolean properties, boolean copy,
			boolean save, boolean print, boolean zoom, boolean ifmove,
			boolean ifback, boolean ifrepain, boolean isadjust)
	{
		...
		if (ifback)
		{
			addMenuItem_Back(result);
		}

		if (zoom)
		{
			addMenuItem_Zoom(result);
		}

		/* 自动调整 */
		if (isadjust)
		{
			addMenuItem_Adjust(result);
		}

		if (ifmove)
		{
			addMenuItem_Move(result);
		}

		if (ifrepain)
		{
			addMenuItem_Recover(result);
		}

		if (copy)
		{
			addMenuItem_Copy(result);
		}

		if (save)
		{
			addMenuItem_Save(result);
		}

		if (print)
		{
			addMenuItem_Print(result);
		}

		if (properties)
		{
			addMenuItem_Properties(result);
		}
	 ...
	 * @return
	 */
	public JMenu createJMenu(boolean properties, boolean copy, boolean save,
			boolean print, boolean zoom, boolean ifmove, boolean ifback,
			boolean ifrepain, boolean isadjust)
	{
